...
export async function updateFounderImages(): Promise<void> {
  const db = getPool();
  const { rows: founders } = await db.query<{ id: number; name: string; image_url: string | null }>(
    'SELECT id, name, image_url FROM founders WHERE image_url IS NULL'
  );
...
export async function computeEloRatings(db: Pool): Promise<Map<number, number>> {
  const { rows: founderRows } = await db.query<{ id: number }>('SELECT id FROM founders');
  const ratings = new Map<number, number>(founderRows.map(r => [r.id, 1500]));

  const { rows: votes } = await db.query<{ winner_id: number; loser_id: number }>(
    'SELECT winner_id, loser_id FROM pairwise_votes ORDER BY created_at ASC'
  );
...
async function seedFounders(db: Pool): Promise<void> {
  ...
  for (const f of founders) {
    ...
    await db.query(
      ...
      [f.name, f.company, f.title, f.bio, sassy_score, f.arrogance, f.controversial_takes, f.interruption_tendency, f.humblebragging, f.buzzword_density]
    );
  }
