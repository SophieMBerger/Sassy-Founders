...
const DB_PATH = path.join(__dirname, '../../data/sassy.db');
...
function seedFounders(db: Database.Database) {
  const count = (db.prepare('SELECT COUNT(*) as c FROM founders').get() as { c: number }).c;
  if (count > 0) return;
  ...
  const insertAll = db.transaction((rows: typeof founders) => {
